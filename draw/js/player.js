    ...
    $.extend(b.prototype, {
        ...
        fire: function(g) {
            ...
            if (this.called === 0) {
                ...
                e = this.defaultHandles.length; f < e; f++) {
...
(function(b) {
    ...
    $.extend(a.prototype, {
        ...
        init: function() {
            var c = this;
            this.app.on("data:courseInfoLoaded",
            function(d) {
    ...
    $.extend(a.prototype, {
        init: function() {
            ...
            $(".player .scrubber").live({
                ...
                mouseout: function(k) {
                    if (f.seekHanding) {
                        $("#page").off("mousemove touchmove", g).on("mousemove touchmove", g).off("mouseup touchend", j).on("mouseup touchend", j)
                    } else {
            ...
            if (fullScreenApi.supportsFullScreen) {
                ...
                $("#stage").on(fullScreenApi.fullScreenEventName,
                function(k) {					
            ...
            $(window).keydown(function(l) {
                if (l.keyCode == 37 || l.keyCode == 39 && l.target.tagName.toLowerCase() != "input") {
                    var m = $(".page-current"),
                    n = ~~ (m.val()),
                    k = $(".page-total").text();
            ...
            function() {
                ...
                e.toolbarDisplay = setInterval(function() {
                    ...
                    if (f.status.display && f.view.app.status.play && (Math.abs(k - f.status.activeTime) > 6000)) {
            ...
            this.view.on("view:progress",
            function(k) {
                if (!f.seekHanding) {
                    f.setProgress(k)
                }
            });
            this.view.on("view:loadedprogress",
            function(k) {
                $(".loaded").width(k * 100 + "%")
            });
            this.view.on("view:lock",
            function(k) {
                f.locked = true
            });
            this.view.on("view:unlock",
            function(k) {
                f.locked = false
            });
            this.view.app.on("timeupdate",
            function(k) {
				//播放时已经播放的时间显示
                $(".played").text(b.player.view.secondToString(k))
            });
            this.view.app.on("data:courseInfoLoaded",
            function(k) {
                $(".duration").text(b.player.view.secondToString(k.totalTime));
                $(".page-total").text(Math.floor(k.maxHeight / k.height) + 1)
            });
            this.view.app.on("audio:volumechange",
            function(l, k) {
                if (k) {
                    l = 0
                }
...
(function(b) {
    ...
    function a(c) {
        ...
        this.isMobile = (function() {
            ...
            if (/android/i.test(e) || (/linux/i.test(e) && /arm/i.test(navigator.platform))) {
                return "android"
            }
            return false
        })();
    ...
    $.extend(a.prototype, {        
        ...
        init: function() {
            ...
            this.updateStageView = function() {
                ...
                var j, g, f = new Date().getTime(),
                e = Math.floor(this.status.top / this.app.info.height) || 0;
                if (! (($(this.canvas.element).data("page") === (e)) && !this.status.pageChanged[e])) {
                ...
                g = ($(this.next.element).data("page") === (e + 1) && !this.status.pageChanged[e + 1]);
                ...
                if (this.app.status.dockview && (this.status.dockviewUpdate || (f - this.dockviewEle.data("updateTime") > 2000))) {
            ...
            if (this.isMobile) {
                ...
                this.out.get(0).addEventListener("touchstart",
                function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                    c.start = e.timeStamp;
                ...
                false);
                this.out.get(0).addEventListener("touchend",
                function(f) {
                ...
                false);
                this.out.get(0).addEventListener("touchmove",
                function(f) {
                    f.preventDefault();
                    f.stopPropagation();
                    c.end = f.timeStamp;
            ...
            this.app.on("page:image:loading",
            function(e) {
                var f = d.out.find("#page-loading-" + e);
                if (!f.get(0)) {
                    d.out.prepend('<img src="http://s.aifudao.com/player/img/loading.gif" data-page="' + e + '" id="page-loading-' + e + '" class="loading" />');
                    d.out.find("#page-loading-" + e).css("top", (e * d.app.info.height + (d.app.info.height / 2) - 16) * d.status.zoomRate)
                }
            });
            this.app.on("page:image:loaded",
            function(e) {
                d.out.find("#page-loading-" + e).remove()
            });
            this.app.on("page:image:error",
            function(e) {
                d.out.find("#page-loading-" + e).remove()
            });
            this.app.on("player:error",
            function(e) {
				//页面文件加载出错时
                $(".error-message").html('<p class="alert-error">' + e + "</p>").removeClass("hide");
				//$(".player").hide();		
                $(".player").show();				
                d.resize();
                $(window).off("keypress")
            ...
            this.app.on("seek",
            function(e) {});
            this.app.on("timeupdate",
            function(f, e) {
				//播放时进度条的改变
                //d.fire("view:progress", Math.round(f / e * 100))
            });
            this.app.on("audio:loadedprogress",
            function(e) {
                d.fire("view:loadedprogress", e)
            });
            this.app.on("resource:SyncResource",
            function(g) {
                if (g.CourseId > 0) {
            ...
            function(l) {
                ...
                d.out.prepend(m);
                m = null;
                var e = $('<div class="pencil" style="display:none"><!-- img src="http://s.aifudao.com/player/img/pencil.png" width="100%" height="100%" --></div>');
                d.out.append(e);
                d.pencil = e;
                d.resize();
                var f = ["<h5>", "<span>老师：<a target='_blank' href='/teacher/info?username=", encodeURIComponent(l.teacher), "'>", l.teacher, "</a></span>", (l.student ? "<span>学生:" + l.student + "</span>": ""), "<span class='time'>时间：", l.startTime, "</span>", "</h5>", " <ul id='player-cover-play'>", " <li><img class='play' src='http://s.aifudao.com/player/img/play.png'></li>", " </ul>", "<ul class='time-snap'>"];
                var j = Math.floor(l.totalTime / 6);
                f.push('<li data-sec="' + (l.totalTime) + '"><span>时长：' + b.player.view.secondToString(l.totalTime, 1) + '</span><img  class="snapshot-img" src="about:blank"></li>');
                f.push("</ul>");
                $(".player-cover").html(f.join("")).removeClass("hide");
                $(".player-cover .time-snap li .snapshot-img").on("load",
                function() {
                    if (!this) {
                        return
                    }
            ...
            this.app.on("data:change",
            function(j, f, i, g) {
                var e = (j[0] + j[2] - d.app.info.height) / 2;
                if (e < 0) {
                    e = 0
                }
                if (j[0] >= d.status.top && j[2] <= d.status.bottom) {} else {
            ...
            this.app.on("page:update",
            function(e) {
                d.status.pageChanged[e] = 1
            });
            this.app.on("dockview:status:close",
            function() {
                d.dockviewEle.hide()
            });
            this.app.on("dockview:status:min",
            function() {
                d.dockviewEle.hide()
            });
            this.app.on("dockview:status:max",
            function() {
                d.dockviewEle.show()
            });
            this.app.on("dockview:status:open",
            function(i) {
                var g = d.dockviewEle;
                var e = i[1] - i[3],
                f = i[2] - i[0];
                ...
                g.width(e * d.status.zoomRate).height(f * d.status.zoomRate).show();
                g.show().data("width", e).data("height", f).data("top", i[0]).data("left", i[3]);
                if (d.app.cache.resource.length > 0) {
                    ...
                    }).html(d.out.find(".resource[data-page=" + k + "],.resource[data-page=" + j + "]").clone())
                }
            });
            this.app.on("data:dockview:update",
            function(f, e) {
            ...
            this.on("view:unlock",
            function() {});
            this.on("view:scroll:done",
            function(e) {});
            this.on("view:scroll:begin",
            function(e) {});
            this.on("view:resize",
            function(e, i) {
                d.stage.width(e).height(i);
                d.out.height((d.status.maxTop + d.app.info.height) * d.status.zoomRate + d.status.toolbarHeight).width(e);
                if (d.app.status.dockview) {
                    ...
                    k.find(".dock-stage").width(e).height(d.out.height()).css({
        ...
        resize: function() {
            ...
            this.status.zoomRate = Math.min(c / this.app.info.width, d / this.app.info.height);
        ...
        loadResource: function(i, j, f, k, m, e) {
            var l = this;
            var c = b.player.config.resourceBaseUrl + "CourseId" + i + "_Index" + j + "/Kejian";
            if (m && m > 1) {
                c += "-" + f
            }
            c += ".jpg";
            ...
            if (($("#" + d).length == 0) && (k <= this.status.maxPage)) {
                var n = $('<div class="resource" data-page="' + k + '"id="' + d + '"></div>');
                this.out.prepend(n);
                n.css({
        ...
        dataloading: function() {
            ...
            this.app.on("data:loading:percent",
            function(d) {
                d = Math.min(1, d);
                if (d > $(".loading-progress .progress").data("percent")) {
                    $(".loading-progress .progress").data("percent", d).clearQueue().animate({
                        width: d * 100 + "%"
                    },
                    "slow", "swing")
                }
                if (d >= 1) {
                    $(".loading-progress .progress").clearQueue().animate({
                        width: "100%"
                    },
                    "fast", "swing").promise().done(function() {
                        $("#loading-cover").hide("slow").promise().done(function() {
                ...
                $("#stage").css("border", "1px solid #ccc").css("border-color", "transparent")
            })
        }
    });
	//显示播放时长
    ...
    $.extend(d.prototype, {
        ...
        line: function(k, f, g, j) {
            var i = this;
            if (this.checkTime(j)) {
                $(this.canvas.element).queue("cmd",
                function() {
                    if (!i) {
                        return
                    }
                    i.canvas.line([k[0] - i.left, k[1] - i.top], [f[0] - i.left, f[1] - i.top], g);
        ...
        image: function(j, l, g, f, k) {
            ...
            this.canvas.image(j, [l[0] - this.left, l[1] - this.top], g, f,
            function() {
                if (!i) {
                    return
                }
        ...
        loadBgImage: function(g) {
            ...
            this.startTime = i;
            this.status.bgImage = this.image(c.courseUrl + $.param({
                sid: this.app.info.courseId,
                action: "snapshot",
                t: i,
                start: this.page + 1
            }), [0, this.top], this.width, this.height)
        },
...
(function(b) {
    ...
    a.isSupport = function() {
        return typeof Audio != "undefined" && typeof new Audio().canPlayType == "function" && (new Audio().canPlayType("audio/mpeg") == "maybe" || new Audio().canPlayType("audio/mpeg") == "probably") ? true: false
    };
    a.prototype = {
        init: function() {
            var c = this;
    ...
    $.extend(a.prototype, {
        init: function(c) {
            ...
            this.timer.cmd = setInterval(function() {
                ...
                if (d.status.play) {
                    ...
                    if (d.info.currentTime + g < d.info.totalTime && !d.cache.cmds[f + 1] && !d.status.cmds[f + 1]) {
                        d.getCmd(d.info.currentTime + g)
                    }
                }
            },
            1000);
            this.on("data:courseInfoLoaded",
            function() {});
            this.on("data:resourceLoaded",
            function(f) {
                $.each(f,
                function(j, m) {
                    if (m.Type == "SyncResource" && m.CourseId > 0) {
                        for (var k = 0,
                        g = ~~ (m.Top / d.info.height); k < m.PageNum; k++) {
            ...
            this.on("data:loading",
            function() {});
            this.on("data:cmd:loadedprogress",
            function(f) {
                if (f >= 1) {
                    f = 1
                }
            ...
            this.view.on("view:seek",
            function(f) {
                var g = f * d.info.totalTime / 100;
                d.seek(g)
            });
            this.stage.on("data:change",
            function(k, f, j, i) {
            ...
            this.on("page:update",
            function(f) {});
            this.cmd.on("cmd:dockview:close",
            function(f) {
                d.status.dockview = 0;
                d.cache.dockviewPos = [0, 0, 0, 0];
                d.fire("dockview:status:close", f)
            });
            this.cmd.on("cmd:dockview:open",
            function(f) {
                d.status.dockview = 1;
                ...
                g = !(i[0] == f.Top && (i[1] == f.Right) && (i[2] == f.Bottom) && (i[3] == f.Left));
                d.cache.dockviewPos = [f.Top, f.Right, f.Bottom, f.Left];
                d.fire("dockview:status:open", d.cache.dockviewPos);
                if (g) {
                    d.fire("data:dockview:update", d.cache.dockviewPos, [Math.floor(f.Top / d.info.height), Math.floor(f.Bottom / d.info.height)])
                }
            });
            this.cmd.on("cmd:dockview:min",
            function(f) {
                d.fire("dockview:status:min", f)
            });
            this.cmd.on("cmd:dockview:max",
            function(f) {
                d.fire("dockview:status:max", f)
            });
            this.cmd.on("cmd:resorce:exec",
            function(f) {
            ...
            this.cmd.on("cmd:SyncRotate:exec",
            function(g) {
        ...
        getCachedCmd: function(e, g) {
            ...
            if (this.cache.cmds[d] && this.cache.cmds[d]["Commands"]) {
                $.each(this.cache.cmds[d]["Commands"],
                function(j, k) {
        ...
        getCourseInfo: function() {
            ...
            var c = this;
            c.status.info = 1;
            $.getJSON(b.player.config.courseUrl, {
        ...
        getDockViewInfo: function() {
            var d = this;
            var c = $.getJSON(b.player.config.courseUrl, {
                sid: this.info.courseId,
                action: "dockview"
            },
            function(e) {
        ...
        fixResourcePageNumber: function(d) {
            var c = this;
            $.getJSON(b.player.config.baseUrl + "classes/get_resource_page_number", {
            ...
            function(g) {
                if (g.res == "0") {
                    ...
                    if (d.Type == "SyncResource" && d.CourseId > 0) {
                        for (var f = 0,
                        e = ~~ (d.Top / c.info.height); f < g.page; f++) {
        ...
        getDockViewCmdByTime: function(e) {
            ...
            for (var f = 0,
            d = this.cache.dockview.length; f < d; f++) {
        ...
        playAudio: function() {			
            ...
            if (this.status.audio === 0) {
                this.audio.play(b.player.config.audioBaseUrl + this.info.audioId + ".mp3")
            } else {
                this.audio.play()				
            }
        },
        ...
        seek: function(e) {
            ...
            if (this.status.audio === 0) {
                ...
                if (e > 0) {
                    for (var f = 0,
                    d = this.cache.resource.length; f < d; f++) {
        ...
        getCmd: function(d) {
            ...
            var e = $.getJSON(b.player.config.courseUrl, {
                sid: this.info.courseId,
                action: "pixel",
                start: d,
            ...
            b.utils.request("/kejian", {
                ...
                ok: function(o) {
                    ...
                    if (f.status.preload.cmd > 0) {
                        f.fire("data:cmd:loadedprogress", f.status.preload.cmdLoaded / f.status.preload.cmd)
                    }
                },
                fail: function() {
