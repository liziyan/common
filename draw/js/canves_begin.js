var c=0,count=0;d=0,z=0,end=0,endx=0,endy=0,hour=0,hour1=0,hour2=0,pauseNum=1,playTime=0;
var t,r,ddd="",ccc="",cc="";
var backNum=0;
var playNum=0;
...
function timedCountsDraw(num){	
	...
	if(myAry[num+1][5]==0 && myAry[num+1][6]==0 && myAry[num+1][7]==0)
	{			
		ctx_begin[cc].beginPath();
		var _a=myAry[num][0];
		if(_a.indexOf("=")>-1)	
		_a=_a.substr(1,_a.length);
...
function backDrawCircle(){				
	...
	{					
		...
		{
			if(myAry[i+1][5]==1)//橡皮差
			{				
				ctx_begin[cc].globalCompositeOperation = "destination-out";
				ctx_begin[cc].beginPath();					
				ctx_begin[cc].arc(myAry[i+1][0]*_pw,myAry[i+1][1]*_ph,15,0, Math.PI * 2);	  
				ctx_begin[cc].strokeStyle="rgba(250,250,250,0)";
				ctx_begin[cc].fill();
				ctx_begin[cc].globalCompositeOperation = "source-over";	
			}
			if(myAry[i+1][7]==1)//清除画布
			{
				ctx_begin[cc].beginPath();					
				ctx_begin[cc].clearRect(0, 0,_canvasWidth,_canvasHeight);
				imgurl();			
...
{  	
	...
	{		
		var imgs=ImgUrl.split("|").length;
		  ...
		  dataString=srcString;			
		  var text="<li><img src='"+dataString+"' data='"+dataString+"' /></li>";	
		  $(".imglist ul").append(text);		 
	  	}				
	}		
	$(".imglist ul li img").css({"width":imgWidth+"px","height":imgHeight+"px"});
...
function EnterPress(e){ 	
  ...
  if(e.keyCode == 13){	 
		...
		if(parseInt($(".page-current").val())>parseInt($(".page-total").text()))
		{
