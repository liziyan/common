...
function canvasPage(canNum){
	...
	$(".canvas").eq(canNum).removeClass("hide").siblings(".canvas").addClass("hide");
...
function onLoadPage(){	
	...
	$(".list").css("left",(canvasW-320)/2+"px").css("top",(_pageHx-320)/2+"px");
	$(".xszd_list").css("left",(canvasW-320)/2+"px").css("top",(_pageHx-320)/2+"px");		
	if(ImgUrl.split("|").length>1)
	{imgWidth=_pageWx/3-40;imgHeight=_pageHx/3;	}
	else
	{imgWidth=_pageWx*9/20-40;imgHeight=_pageHx*2/5;}
}
//添加画布
...
function getTimes(){
	var myDate = new Date();
	var dateTime=myDate.getHours()*3600000 + myDate.getMinutes()*60000 + myDate.getSeconds()*1000 + myDate.getMilliseconds();
	return dateTime;
}
